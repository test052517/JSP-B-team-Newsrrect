...
%>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <form id="approvalForm" action="processPost.jsp" method="post">
                ...
                <div class="p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <div id="postContent" class="w-full px-4 py-6 bg-gray-50 border border-gray-200 rounded-lg text-gray-800 min-h-[200px] leading-relaxed">
        ...
        <div class="flex justify-center space-x-4 mb-8">
            ...
            <button onclick="rejectPost()" class="group relative bg-red-600 text-white px-10 py-4 rounded-lg hover:bg-red-700 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                <span class="flex items-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    <span>거절</span>
                </span>
            </button>
            <button onclick="goBack()" class="group relative bg-gray-500 text-white px-10 py-4 rounded-lg hover:bg-gray-600 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                <span class="flex items-center space-x-2">
    ...
    <div id="approvalModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full transform transition-all">
            <div class="p-6">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2 text-center">게시글 승인</h3>
                <p class="text-gray-600 mb-6 text-center">이 게시글을 승인하시겠습니까?<br><span class="text-sm text-gray-500">승인 시 뉴스 분석 팝업이 열립니다.</span></p>
                <div class="flex space-x-3">
                    <button onclick="closeModal()" class="flex-1 px-4 py-3 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors font-medium">취소</button>
                    <button onclick="confirmApproval()" class="flex-1 px-4 py-3 text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors font-medium">승인</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full transform transition-all">
            <form id="rejectionForm" action="processPost.jsp" method="post">
                ...
                <div class="p-6">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 mb-2 text-center">게시글 거절</h3>
                    <p class="text-gray-600 mb-4 text-center text-sm">거절 사유를 입력해주세요.</p>
                    <div class="mb-4">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">거절 사유 <span class="text-red-500">*</span></label>
                        <textarea name="rejectionReason" class="w-full border border-gray-300 rounded-lg p-3 h-32 resize-none focus:ring-2 focus:ring-red-500 focus:border-red-500 focus:outline-none transition-all" placeholder="거절 사유를 상세히 입력해주세요..." required></textarea>
                    </div>
                    <div class="flex space-x-3">
                        <button type="button" onclick="closeModal()" class="flex-1 px-4 py-3 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors font-medium">취소</button>
                        <button type="submit" class="flex-1 px-4 py-3 text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors font-medium">거절</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function isUrl(str) {
            ...
            var domainPattern = /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/i;
            str = str.trim();
