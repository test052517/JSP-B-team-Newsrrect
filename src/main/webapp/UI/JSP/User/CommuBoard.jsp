...
%>
<%
    // InfoBoard.jsp와 동일한 로직을 사용하여 '소통' 타입 게시글을 로드합니다.
    ...
    // 타입: '소통'으로 설정
    totalRecord = watchUserPostMgr.getTotalCount("소통", keyField, keyWord);
    ...
    // 타입: '소통'으로 설정
    Vector<PostBean> postList = watchUserPostMgr.getPostList("소통", keyField, keyWord, start, numPerPage);
...
<body class="bg-white min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="p-6">
                <!-- 검색 폼 -->
                <form name="searchFrm" method="get" action="CommuBoard.jsp">
                    ...
                    <c:choose>
                        <c:when test="${not empty postList}">
                            <c:forEach var="post" items="${postList}" varStatus="status">
                                <%-- 서블릿의 URL 매핑 주소로 변경 --%>
								<c:url var="watchUrl"
									value="/commu/watch.do">
									<c:param name="id" value="${post.postId}" />
									<c:param name="nowPage" value="${nowPage}" />
								</c:url>
								<a href="${watchUrl}"...> <a href="${watchUrl}"
									class="grid grid-cols-12 gap-4 py-3 px-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors duration-200 cursor-pointer">
										<div class="col-span-1 text-sm text-gray-900">${totalRecord - ((nowPage - 1) * numPerPage) - status.index}</div>
										<div
											class="col-span-7 text-sm text-gray-900 font-medium truncate">${post.title}</div>
                ...
                <div class="flex justify-center items-center mt-6 space-x-2">
                    <c:if test="${nowBlock > 1}">
                        <c:url var="prevUrl" value="CommuBoard.jsp">
                    ...
                    <c:forEach begin="${pageStart}" end="${pageEnd}" var="i">
                        <c:url var="pageUrl" value="CommuBoard.jsp">
                    ...
                    <c:if test="${nowBlock < totalBlock}">
                         <c:url var="nextUrl" value="CommuBoard.jsp">
